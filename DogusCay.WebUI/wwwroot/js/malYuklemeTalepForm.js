...
//    // Eklenen ürüne veya tabloya otomatik scroll (mobil UX için)
//    setTimeout(() => {
//        // Son eklenen ürünün satırına scroll et
//        const lastProduct = addedProducts[addedProducts.length - 1];
//        const lastRow = document.getElementById(`product-row-${lastProduct.productId}`);
//        if (lastRow) {
//            lastRow.scrollIntoView({ behavior: 'smooth', block: 'center' });
//        } else {
//            // Eğer satır bulunamazsa tüm tabloya scroll et
...
//            html += `
//                <div class="cart-product-card" id="product-row-${p.productId}">
//                    <div class="prd-title">${index + 1}. ${p.productName}</div>
//                    <div class="prd-row">
//                        <span class="prd-label">Fiyat:</span>
//                        <span class="prd-value">${p.price !== undefined && p.price !== null ? formatTL(p.price) : '-'}</span>
//                    </div>
//                    <div class="prd-row">
//                        <span class="prd-label">Miktar:</span>
//                        <button class="btn btn-sm btn-outline-secondary" type="button" onclick="updateProductQuantity(${p.productId}, -1)">-</button>
//                        <input type="number" min="1" value="${p.quantity || 0}" style="width:48px;display:inline-block;text-align:center;" onchange="setProductQuantity(${p.productId}, this.value)" />
//                        <button class="btn btn-sm btn-outline-secondary" type="button" onclick="updateProductQuantity(${p.productId}, 1)">+</button>
//                    </div>
//                    <div class="prd-row">
//                        <span class="prd-label">İskonto(%):</span>
//                        <input type="number" min="0" max="100" value="${p.discount || 0}" style="width:52px" onchange="setProductDiscount(${p.productId}, this.value)" />
//                    </div>
//                    <div class="prd-row">
//                        <span class="prd-label">Sabit Fiyat:</span>
//                        <input type="number" min="0" value="${p.fixedPrice || ''}" style="width:68px" onchange="setProductFixedPrice(${p.productId}, this.value)" />
//                    </div>
...
//        renderProductCards(); // Mobil: Kart görünüm
//    } else {
//        renderProductTable(); // Masaüstü: Tablo
//    }
//}

//// Her ekleme/çıkarma/indirim vs. işleminde:
//renderSepet();
            ...
            html += `
                <div class="cart-product-card${errorMsg ? " card-error" : ""}" id="product-row-${p.productId}">
                    ...
                    <div class="prd-row">
                        ...
                        <button class="btn btn-sm btn-outline-secondary" type="button" onclick="updateProductQuantity(${p.productId}, -1)">-</button>
                        <input type="number" min="1" value="${quantity}" style="width:48px;display:inline-block;text-align:center;" onchange="setProductQuantity(${p.productId}, this.value)" />
                        <button class="btn btn-sm btn-outline-secondary" type="button" onclick="updateProductQuantity(${p.productId}, 1)">+</button>
                    </div>
                    <div class="prd-row">
                    ...
                    <div class="prd-row">
                        ...
                        <input type="number" min="0" max="100" value="${discount1}" style="width:52px" onchange="setProductDiscount(${p.productId}, this.value)" />
                    </div>
                    <div class="prd-row">
                        <span class="prd-label">İskonto2 (%):</span>
                        <input type="number" min="0" max="100" value="${discount2}" style="width:52px" onchange="setProductDiscount2(${p.productId}, this.value)" />
                    </div>
                    <div class="prd-row prd-actions">
                        <button class="btn btn-sm btn-danger" onclick="removeProduct(${p.productId})">Sil</button>
                    </div>
